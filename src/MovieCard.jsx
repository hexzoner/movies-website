...
export default function MovieCard({ movie, Genres }) {
  ...
  return (
    <div className="text-white bg-[#21242d] rounded-[18px] border-opacity-0 hover:cursor-pointer hover:border-opacity-40 border-solid border-blue-600 border-2 relative">
      ...
      <div className="pointer-events-none absolute bottom-0 w-full rounded-b-[18px] h-full flex flex-col justify-between">
        <div>
          <p id="movie-title" className="font-bold text-l text-center line-clamp-1  bg-black bg-opacity-70 rounded-t-[18px]  ">
            {movie.title}
          </p>
          <div className="flex justify-between bg-black bg-opacity-70 w-full">
        ...
        <div className="flex justify-between items-center bg-black bg-opacity-70  rounded-b-[18px] py-2">
          ...
          <span className="font-semibold text-sm text-right text-[#00b9ae] pr-3">{genresString}</span>
        </div>
      </div>
    </div>
  );
